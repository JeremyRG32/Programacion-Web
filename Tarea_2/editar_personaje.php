...
if ($_POST) {
    ...
    for ($i = 0; $i < count($obra->personajes); $i++) {
        if ($obra->personajes[$i]->cedula == $cedula) {
            $obra->personajes[$i] = $personaje;
            break;
        }
    }
    file_put_contents('datos/' . $codigo . '.json', json_encode($obra, JSON_PRETTY_PRINT));
?>
...
<div style="background-color: rgba(109, 108, 108, 0.33); border-radius: 10px; " class="container-xl p-5">
    <div class="row">
        <div style="background-color:white; border-radius: 10px; margin-top: 40px; max-height: 500px;" class="container p-3 mx-0 col-md-4">
            ...
            <img class="mt-3 rounded-3 mx-auto d-block" style="height: 200px;" src="<?= $personaje->foto ?>">
            <h3 class="mt-1 text-center">Nombre</h3>
            <p class="text-center" style="font-weight:normal; font-size: large;"><?= $personaje->nombre . " " . $personaje->apellido ?></p>
        </div>
        <div class="col-md-8">
            <div class="row">
                <h3 class="text-center" style="font-weight: bold;">Editar un Personaje</h3>
                <form action="editar_personaje.php?codigo=<?= $_GET['codigo'] ?>&cedula=<?= $_GET['cedula'] ?>" method="POST">
                    <?php if ($_POST) {
                    ?>
                    ...
                    <input type="text" name="cedula" class="form-control" value="<?= $personaje->cedula ?>" hidden required>
